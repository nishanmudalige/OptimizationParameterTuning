...
function main()
    ...
    filename = "../results/complete_scalable.csv"
    ...
    for scalable_problem in scalable_problems
        ...
        for mem in r.lower:r.upper
            for i in 0:is_init_run
                ...
                try
                    _, init_eval_obj_time, init_eval_obj_mem, _, init_eval_obj_gcstats = @timed obj(nlp, nlp.meta.x0)
                    _, init_eval_grad_time, init_eval_grad_mem, _, init_eval_grad_gcstats = @timed grad(nlp, nlp.meta.x0)
                    stats, time, memory, _, _ = @timed JSOSolvers.lbfgs(nlp; mem = mem, max_time=MAX_TIME)
                    init_eval_obj_alloc = init_eval_obj_gcstats.poolalloc + init_eval_obj_gcstats.bigalloc
                    init_eval_grad_alloc = init_eval_grad_gcstats.poolalloc + init_eval_grad_gcstats.bigalloc
