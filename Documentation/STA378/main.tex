...
Let \(x\) denote a problem instance identified by \((\texttt{problem}, \texttt{nvar})\).
Given a trained runtime regressor \(\widehat{T}(x,m)\) and candidate set
\(m \in \{1,\dots,100\}\), we select
\[
...
\[
r(x) = \frac{\widehat{T}\!\left(x,\hat{m}(x)\right)}{T\!\left(x,m^\star(x)\right)},
\]
where \(T(x,m)\) is the observed wall-clock runtime. Since the numerator is model-predicted,
\(r(x)\) may be below \(1\); smaller values indicate that the selector is predicted to be
closer to the oracle-best configuration.

Table~\ref{tab:selection} reports the resulting exact match accuracy and ratio-based performance metrics on the test set.
