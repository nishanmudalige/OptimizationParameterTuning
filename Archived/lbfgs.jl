...
function main()
    ...
    problem_names =
        meta[meta.contype .== :unconstrained.&&.!meta.has_bounds.&&meta.nvar .>= 5, :name]; # LBFGS is an algorithm to solve unconstrained problems
        ...
        for mem = r.lower:r.upper
            for i = 0:is_init_run
                ...
                try
                    ...
                    reset!(nlp)
                    bench =
                        @benchmark JSOSolvers.lbfgs($nlp; mem = $mem, max_time = MAX_TIME)
                    reset!(nlp)
