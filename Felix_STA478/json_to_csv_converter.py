...
error_cols = ["error_problem", "error_type", "error_solver"]
...
solver_df_all = pd.read_csv("./datasets/solver_runtimes.csv")
problem_df_all = pd.read_csv("./datasets/problem_metadata.csv")
...
df["timed_memory"] = df["timed_bytes"] / 1e6
df.loc[df["error_solver"].notna(), "stats_elapsed_time"] = MAX_TIME * 2
df.loc[df["error_solver"] == "Timeout after 3600.0 s", "stats_elapsed_time"] = MAX_TIME #3599.2, 3601.2
df.to_csv("./datasets/new_raw.csv", index=False)
final_df = df[df[error_cols].isna().all(axis=1)]
final_df.to_csv("./datasets/new_final.csv", index=False)
